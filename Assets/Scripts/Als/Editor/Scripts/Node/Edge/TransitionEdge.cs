...
namespace Moyu.Anim
{
    public class TransitionEdge : Edge
    {
        ...
        private void BuildContextualMenu(ContextualMenuPopulateEvent evt)
        {
            ...
            {
                evt.menu.AppendAction("Delete Transition", (dropdownMenuAction) =>
                {
                    DeleteTransition();
                });
            }
        ...
        {
            // input 是箭头指向的结点
            if (output.node is StateNodeView<EntryStateNode> entryNodeView)
            {
                if (input.node is StateNodeView<StateNode> outStateNodeView)
                {
                    entryNodeView.RemoveOutputTransition(outStateNodeView);
                }
            }

            if (output.node is StateNodeView<StateNode> stateNodeView)
            {
                if (input.node is StateNodeView<StateNode> outStateNodeView)
                {
                    stateNodeView.RemoveOutputTransition(outStateNodeView);
                }
            }
        }
