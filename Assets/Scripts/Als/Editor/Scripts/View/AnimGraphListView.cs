...
public class AnimGraphListView : GraphElement
{
    ...
    private void BuildContextualMenu(ContextualMenuPopulateEvent obj)
    {
        ...
        obj.menu.AppendAction("Add StateMachine", action =>
        {
            StateMachine stateMachine = new();
            stateMachine.Name = "StateMachine" + _editor.GetGraphAsset().GetGraphs().Count;
            _editor.GetGraphAsset().GetGraphs().Add(stateMachine);
            listView.Rebuild();
        });
    }
    ...
    private void BindItem(VisualElement element, int index)
    {
        ...
        if (animGraph is StateAnimGraph)
        {
            textField.SetValueWithoutNotify(animGraph.Name + "(State)");
        }
        else
        {
        ...
        textField.style.width = Length.Percent(90);
        ...
        textField.parent.UnregisterCallback<MouseDownEvent>(GraphNameMouseEvent);
        ...
        textField.parent.UnregisterCallback<ContextualMenuPopulateEvent>(Callback);
        ...
        textField.UnregisterCallback<FocusOutEvent>(EventCallback);
        ...
        textField.UnregisterValueChangedCallback(TextFieldValueChanged);
